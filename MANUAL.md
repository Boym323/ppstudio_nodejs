...
- Admin detail rezervace má panel `Úhrada` s jasnou prioritou `Stav úhrady -> Cena k úhradě -> + Zapsat platbu -> + Uplatnit voucher -> Přehled úhrad`. Horní souhrn zůstává nejvýraznější a vždy ukazuje stav (`Bez úhrady` / `Částečně uhrazeno` / `Zaplaceno` / `Přeplaceno`), cenu k úhradě, uhrazeno celkem, voucher, mimo voucher a doplatek nebo přeplatek. Úprava individuální ceny se otevírá kompaktně přes `Upravit` přímo u položky `Cena k úhradě`; samostatný blok `Cena rezervace` se ve výchozím zobrazení nepoužívá. CTA `+ Zapsat platbu` je primární, `+ Uplatnit voucher` je záměrně sekundární a prázdný stav `Přehled úhrad` používá jemnou informaci `Žádné úhrady zatím nejsou evidované.`
