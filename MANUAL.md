...
- Sekce `Zdroje návštěv` v tomto widgetu kombinuje Matomo kampaně a referrer typy do business názvů `Instagram`, `Firmy`, `Google`, `Přímý vstup` nebo `Ostatní`; rezervace u zdrojů jsou výslovně jen orientační odhad podle podílu návštěv na dokončených `Booking / Created`, ne přesná atribuce.
...
- Admin detail rezervace má panel `Úhrada`: nahoře je kompaktní settlement blok s badge stavem úhrady, kde se zřetelně ukazuje cena služby, uhrazeno voucherem, uhrazeno mimo voucher, celkem uhrazeno a zbývající doplatek nebo přeplatek. Sekce `Dárkový poukaz` je civilnější a historie úhrad je vedená jako krátký provozní seznam. U rezervace bez uvedeného voucheru se formulář otevírá až přes `+ Uplatnit voucher`; jakmile je na rezervaci jeden voucher skutečně uplatněný nebo doplatek vychází na nulu, další voucher se v UI nenabízí.
- Platební část `CRM souhrnu` v detailu klientky používá stejný helper `getBookingPaymentSummary(...)` jako detail rezervace. Do `Uhrazeno` a `Neuhrazeno` zahrnuje jen rezervace ve stavech `PENDING`, `CONFIRMED` a `COMPLETED`; `CANCELLED` a `NO_SHOW` se do doplatků nezapočítávají.
...
- Detail voucheru je read-only na `/admin/vouchery/[voucherId]` a `/admin/provoz/vouchery/[voucherId]`. Nahoře má jednu kompaktní summary kartu s kódem, typem, stavem, platností, čerpáním a akcemi `Stáhnout PDF`, `Tisk A4` a `Poslat e-mailem`. Pod ní je dvousloupcový provozní layout: vlevo `Parametry voucheru` a `Historie uplatnění`, vpravo `Kupující a odeslání` a nízká sekce `Poslední e-mailové pokusy`. Tlačítko `Stáhnout PDF` vede na `/admin/vouchery/[voucherId]/pdf` nebo `/admin/provoz/vouchery/[voucherId]/pdf` a stáhne původní aktuálně vygenerovaný dárkový poukaz používaný i pro e-mail. Vedle něj je samostatný odkaz `Tisk A4`, který stáhne A4 arch na výšku s voucherem v horní třetině; zbytek stránky mimo voucher zůstává bílý pro úsporný a čistý tisk. Editace, rušení a mazání zatím nejsou dostupné; provozní uplatnění probíhá z detailu konkrétní rezervace.
...
Reschedule E2E scénář po ověření runtime kolize přepíná na předem určený nekolizní slot z fixture, ne na "další" tlačítko podle pořadí. Při rozšiřování fixture proto drž explicitní cílové labely nebo ISO start časy, aby test zůstal order-independent a stabilní v CI i lokálně.
