...
- Aktuální release je `0.3.13`; řada `0.x.y` znamená před prvním stabilním vydáním, ale i tak držíme stejnou disciplínu změn.
...
- Rezervační flow posílá pouze neosobní eventy `Rezervace / Služba vybrána`, `Datum vybráno`, `Čas vybrán`, `Kontakt zahájen` a po úspěchu `Vytvořena`; self-service změna termínu posílá bezpečné `Rezervace / Datum vybráno` a `Rezervace / Čas vybrán`. `Kontakt zahájen` se odešle až při první interakci s kontaktním polem. Jméno, e-mail, telefon, poznámka ani tokeny se do analytics neposílají.
...
- Blok `Vyžaduje pozornost` se vykreslí jen když existuje alespoň jeden actionable alert (`emphasis !== ok`); pokud nic nehoří, blok se úplně skryje.
...
- Actionable alerty v `Vyžaduje pozornost` aktuálně pokrývají: čekající rezervace na potvrzení, selhané e-maily, chybějící publikované termíny na dnes/zítra, dnešní rozvrh bez volného okna při aktivních rezervacích, rezervace po konci termínu čekající na uzavření a nízkou týdenní kapacitu.
...
- Admin detail rezervace má panel `Úhrada` s jasnou prioritou `Stav úhrady -> Cena k úhradě -> + Zapsat platbu -> + Uplatnit voucher -> Přehled úhrad`. Horní souhrn zůstává nejvýraznější a vždy ukazuje stav (`Bez úhrady` / `Částečně uhrazeno` / `Zaplaceno` / `Přeplaceno`), cenu k úhradě, uhrazeno celkem, voucher, mimo voucher a doplatek nebo přeplatek. Úprava individuální ceny se otevírá kompaktně přes `Upravit` přímo u položky `Cena k úhradě`; samostatný blok `Cena rezervace` se ve výchozím zobrazení nepoužívá. CTA `+ Zapsat platbu` je primární a seznam existujících plateb se zobrazuje jen jednou v `Přehled úhrad`, kde je dostupné i smazání platby. `+ Uplatnit voucher` je sekundární akce přímo v kompaktním voucher bloku a prázdný stav `Přehled úhrad` používá jemnou informaci `Žádné úhrady zatím nejsou evidované.`
...
- Smoke E2E kontroluje, že veřejné canonical/OG URL používají stejnou URL pro danou stránku a že `robots.txt` se `sitemap.xml` nepouští historickou `http://ppstudio.cz` variantu; produkční canonical origin má zůstat `https://ppstudio.cz`.
