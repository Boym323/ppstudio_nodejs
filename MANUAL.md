...
- Admin detail rezervace má panel `Úhrada` s jasnou prioritou `Stav úhrady -> + Zapsat platbu -> voucher -> cena rezervace -> přehled úhrad`. Horní souhrn zůstává nejvýraznější a vždy ukazuje stav (`Bez úhrady` / `Částečně uhrazeno` / `Zaplaceno` / `Přeplaceno`), cenu k úhradě, uhrazeno celkem, voucher, mimo voucher a doplatek nebo přeplatek. CTA `+ Zapsat platbu` je primární, `+ Uplatnit voucher` je záměrně sekundární. `Cena rezervace` je tlumenější sekundární blok a prázdný stav `Přehled úhrad` používá jemnou informaci `Žádné úhrady zatím nejsou evidované.`
