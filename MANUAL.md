...
- Meta Pixel nad rámec `PageView` sleduje i klíčové neosobní funnel kroky: `ViewContent` na detailu služby, `InitiateCheckout` až po výběru termínu nebo při přechodu do kontaktního kroku, `AddToCart` při výběru služby, custom `BookingDateSelected` / `BookingTimeSelected` / `BookingContactStarted` a po úspěchu `Lead`.
