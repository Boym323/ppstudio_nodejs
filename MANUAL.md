...
- Detail voucheru je read-only na `/admin/vouchery/[voucherId]` a `/admin/provoz/vouchery/[voucherId]`. Nahoře má jednu kompaktní summary kartu s kódem, stavem, typem, hodnotou nebo službou, platností, čerpáním a akcemi; údaje kupujícího/obdarovaného jsou zhuštěné do vyplněných polí, historie uplatnění je tabulková nebo kartová podle šířky obrazovky a interní poznámka zůstává dole. Tlačítko `Stáhnout PDF` vede na `/admin/vouchery/[voucherId]/pdf` nebo `/admin/provoz/vouchery/[voucherId]/pdf` a stáhne aktuálně vygenerovaný dárkový poukaz. Editace, rušení a mazání zatím nejsou dostupné; provozní uplatnění probíhá z detailu konkrétní rezervace.
