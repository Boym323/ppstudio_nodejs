...
- Aktuální release je `0.1.11`; řada `0.x.y` znamená před prvním stabilním vydáním, ale i tak držíme stejnou disciplínu změn.
...
- Admin detail rezervace má panel `Úhrada`: nahoře je blok `Cena rezervace` a kompaktní settlement blok s badge stavem úhrady, kde se zřetelně ukazuje cena k úhradě, uhrazeno voucherem, uhrazeno mimo voucher, celkem uhrazeno a zbývající doplatek nebo přeplatek. Sekce `Dárkový poukaz` je civilnější a historie úhrad je vedená jako krátký provozní seznam. U rezervace bez uvedeného voucheru se formulář otevírá až přes `+ Uplatnit voucher`; jakmile je na rezervaci jeden voucher skutečně uplatněný nebo doplatek vychází na nulu, další voucher se v UI nenabízí.
- `OWNER` i `SALON` mohou v panelu `Úhrada` nastavit individuální cenu rezervace přes `Upravit cenu`. Prázdná hodnota nebo stejná částka jako ceníkový snapshot úpravu zruší; rozdílná cena vyžaduje důvod. Sleva ani navýšení nejsou platba, ale mění cenu k úhradě, ze které se počítá voucher, běžné platby, doplatek i CRM souhrn.
