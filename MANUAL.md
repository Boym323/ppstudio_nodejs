...
- Prodejní aktivace je dostupná přes `Aktivovat voucher` nebo `/admin/vouchery/aktivovat` a `/admin/provoz/vouchery/aktivovat`. Obsluha nejdřív načte kód z předtištěného kusu, vybere hodnotu nebo službu a potvrdí `Potvrdit prodej a aktivovat`. Aktivace vytvoří běžný aktivní voucher se stejným kódem a uloží vazbu na skladový kus; opakované odeslání je bezpečně idempotentní.
