...
- Matomo bootstrap na veřejném webu běží přes inline `next/script` s `afterInteractive`, aby tokenové self-service stránky měly bezpečnou `_paq` queue připravenou spolehlivě hned po interaktivitě. Následná App Router navigace se dopočítává v `MatomoTracker` efektu a raw tokenové URL se dál nikdy neposílají.
