...
- Admin detail rezervace má panel `Úhrada`: nahoře read-only settlement box ukazuje cenu služby, uhrazeno voucherem, zbývá doplatit a stav `Neuhrazeno / Částečně uhrazeno / Uhrazeno`, přičemž doplatek je vizuálně nejvýraznější. Sekce `Dárkový poukaz` dál řeší intended voucher, ruční zadání kódu a historii úhrad. U rezervace bez uvedeného voucheru se formulář otevírá až přes `+ Uplatnit voucher`; jakmile je na rezervaci jeden voucher skutečně uplatněný nebo doplatek vychází na nulu, další voucher se v UI nenabízí.
