...
- Aktuální release je `0.7.1`; řada `0.x.y` znamená před prvním stabilním vydáním, ale i tak držíme stejnou disciplínu změn.
...
- Matomo skript na veřejném webu je záměrně odložený přes `lazyOnload`, aby se homepage nejdřív vykreslila s minimem klientské práce na hlavním vlákně. Samotný `_paq` bootstrap ale běží po hydraci v `MatomoTracker`, takže i tokenové self-service stránky mají včas připravenou bezpečnou queue pro neosobní eventy bez čekání na idle inline snippet.
