...
function HeroSection() {
  ...
  return (
    <section className="relative isolate overflow-hidden border-b border-black/5 bg-[radial-gradient(circle_at_top_left,rgba(226,205,182,0.5),transparent_32%),linear-gradient(180deg,#f8f2eb_0%,#f5ede4_48%,#f8f3ed_100%)]">
      <Container className="grid gap-8 py-10 sm:gap-10 sm:py-14 lg:grid-cols-[1.02fr_0.98fr] lg:items-center lg:gap-14 lg:py-20">
        <div className="space-y-6 lg:space-y-7">
          <div className="space-y-4">
            <p className="text-eyebrow tracking-[0.22em] text-[var(--color-accent)]">{profile.eyebrow}</p>
            <h1 className="max-w-3xl whitespace-pre-line font-display text-[2.65rem] leading-[1.02] tracking-tight text-[var(--color-foreground)] sm:text-5xl lg:text-[4.25rem]">
              {profile.headline}
            </h1>
            <p className="max-w-2xl text-[15px] leading-7 text-[var(--color-muted)] sm:text-lg sm:leading-8">
              {profile.intro}
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-2.5">
            {profile.badges.map((badge) => (
              <span
                key={badge}
                className="rounded-full border border-[var(--color-accent)]/30 bg-[#fff8f1] px-4 py-2 text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--color-accent-contrast)] sm:text-[12px]"
              >
                {badge}
              </span>
            ))}
        ...
        <aside className="flex">
          {profile.image ? (
            <div className="relative w-full overflow-hidden rounded-[var(--radius-panel)] border border-white/80 bg-white/70 shadow-[var(--shadow-panel)] backdrop-blur">
              <Image
                src={profile.image.src}
                alt={profile.image.alt}
                width={profile.image.width}
                height={profile.image.height}
                className="h-[22rem] w-full object-cover object-center sm:h-[28rem] lg:h-[36rem]"
                priority
              />
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {whyChooseMe.items.map((item, index) => (
            ...
            >
              ...
              <h3 className="mt-5 font-display text-[1.65rem] leading-[1.06] text-[var(--color-foreground)]">
                {item.title}
              </h3>
            </article>
          ))}
...
function StorySection() {
  ...
  return (
    <section className="py-10 sm:py-14 lg:py-16">
      <Container>
        <article className="rounded-[var(--radius-panel)] border border-black/6 bg-white p-6 shadow-[var(--shadow-panel)] sm:p-8 lg:p-10">
          <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:gap-12">
            <SectionHeading eyebrow={story.eyebrow} title={story.title} />
            <div className="space-y-5 text-[15px] leading-7 text-[var(--color-muted)] sm:text-base">
  ...
  return (
    <section className="py-10 sm:py-14 lg:py-16">
      <Container className="grid gap-6 lg:grid-cols-[0.92fr_1.08fr] lg:items-start">
        <div className="rounded-[var(--radius-panel)] border border-black/6 bg-white p-6 shadow-[var(--shadow-panel)] sm:p-8 lg:sticky lg:top-24">
        ...
        <div className="grid gap-4">
          {approach.values.map((value) => (
            <article
              key={value.title}
              className="rounded-[var(--radius-panel)] border border-[#e6dbcf] bg-[linear-gradient(180deg,#fffcf8_0%,#fbf4eb_100%)] p-6 shadow-[var(--shadow-panel)] sm:p-7"
            >
              <h3 className="font-display text-[2rem] leading-none text-[var(--color-foreground)]">{value.title}</h3>
...
function WhatToExpectSection() {
  ...
  return (
    <section className="py-10 sm:py-14 lg:py-16">
      <Container className="grid gap-6 lg:grid-cols-[1.06fr_0.94fr] lg:items-start">
        ...
        <aside className="lg:sticky lg:top-24">
          <div className="rounded-[var(--radius-panel)] border border-[#e2d4c4] bg-[linear-gradient(180deg,#f9f3eb_0%,#f5ede4_100%)] p-6 shadow-[0_18px_45px_rgba(67,45,29,0.06)] sm:p-8">
      ...
      <Container>
        <div className="rounded-[var(--radius-panel)] bg-[linear-gradient(135deg,#1b1613_0%,#2a211b_55%,#3a2f28_100%)] px-5 py-7 text-white shadow-[0_24px_70px_rgba(34,22,12,0.18)] sm:px-8 sm:py-10 lg:flex lg:items-center lg:justify-between lg:gap-8">
          <div className="max-w-2xl">
            ...
            <h2 className="mt-4 font-display text-3xl leading-[1.08] text-white sm:text-4xl">
          ...
          <div className="mt-7 flex flex-col gap-3 sm:flex-row lg:mt-0">
            <Link
              href={cta.primaryCta.href}
              className="inline-flex min-h-13 items-center justify-center rounded-full bg-[var(--color-accent)] px-7 py-4 text-[11px] font-semibold uppercase tracking-[0.16em] text-[var(--color-accent-contrast)] hover:brightness-105 sm:text-sm"
            >
              {cta.primaryCta.label}
            </Link>
            <Link
              href={cta.secondaryCta.href}
              className="inline-flex min-h-13 items-center justify-center rounded-full border border-white/15 bg-white/6 px-7 py-4 text-[11px] font-semibold uppercase tracking-[0.16em] text-white hover:border-white/30 hover:bg-white/10 sm:text-sm"
            >
              {cta.secondaryCta.label}
            </Link>
          </div>
