...
export async function preflightVoucherTemplateMaster(bytes: Buffer): Promise<VoucherTemplatePreflight> {
  ...
  const errors: string[] = [];
  let pdf: PDFDocument;
  try { pdf = await PDFDocument.load(bytes, { ignoreEncryption: false }); } catch { return { pageCount: 0, mediaBox: null, trimBox: null, bleedBox: null, encrypted: false, geometryValid: false, outputIntentPresent: false, pdfXClaim: null, pdfXVerification: "NOT VERIFIED", warnings, errors: ["PDF nelze načíst nebo je chráněné heslem."] }; }
  ...
  return { pageCount, mediaBox, trimBox, bleedBox, encrypted: false, geometryValid, outputIntentPresent, pdfXClaim, pdfXVerification: "NOT VERIFIED", warnings, errors };
}
