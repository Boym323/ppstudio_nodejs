...
export function AdminWeeklyPlannerClient({ data, weekStart, initialDate, routeBase }: { data: PlannerWeekData; weekStart: string; initialDate?: string; routeBase: string }) {
  ...
  useEffect(() => { saveQueueRef.current = new PlannerLabSaveQueue(
    ...
    setIsSaving,
    (savedChange) => { confirmedDaysRef.current = cloneWeekDays(savedChange.days); },
    () => { setSaveError(null); setMessage("Uloženo"); },
  ...
  useEffect(() => {
    if (data.weekKey !== requestedWeekRef.current || (data.weekKey === hydratedWeekRef.current && !restoreRequestedRef.current)) return;
  ...
  function restoreSavedState() { saveQueueRef.current?.discardPending(); setDays(cloneWeekDays(confirmedDaysRef.current)); setSaveError(null); setMessage("Uloženo"); restoreRequestedRef.current = true; router.refresh(); }
