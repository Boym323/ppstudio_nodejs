...
export function AdminBookingsQuickActions({
  ...
  return (
    <div className="space-y-1">
      <form
        action={formAction}
        className="inline-flex flex-wrap items-center justify-end gap-1 rounded-full border border-white/8 bg-white/[0.03] p-1"
      >
        ...
        <Link
          href={href}
          className="inline-flex min-h-8 items-center rounded-full border border-white/12 px-3 py-1 text-[11px] font-medium text-white/72 transition hover:border-white/24 hover:bg-white/7 hover:text-white"
        >
          Detail
        </Link>
      </form>
...
function QuickSubmitButton({
  ...
  return (
    <button
      ...
      disabled={pending}
      className={cn(
        "inline-flex min-h-8 items-center rounded-full border px-3 py-1 text-[11px] font-medium transition disabled:cursor-not-allowed disabled:opacity-60",
        value === "CONFIRMED"
