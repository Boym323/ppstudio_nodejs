...
function FilterFields({ data }: { data: AdminLogsData }) { return <><select name="severity" aria-label="Závažnost" defaultValue={data.filters.severity} className="min-h-11 rounded-lg border border-white/10 bg-black/20 px-3 text-sm text-white"><option value="all">Závažnost</option><option value="info">Informace</option><option value="success">V pořádku</option><option value="warning">Varování</option><option value="error">Chyba</option></select>{data.view === "events" ? <select name="source" aria-label="Zdroj" defaultValue={data.filters.source} className="min-h-11 rounded-lg border border-white/10 bg-black/20 px-3 text-sm text-white"><option value="all">Zdroj</option><option value="booking">Rezervace</option><option value="voucher">Voucher</option><option value="service">Služby</option>{data.area === "owner" ? <option value="settings">Nastavení</option> : null}<option value="availability">Dostupnost</option></select> : null}{data.view === "system" ? <select name="source" aria-label="Zdroj" defaultValue={data.filters.source} className="min-h-11 rounded-lg border border-white/10 bg-black/20 px-3 text-sm text-white"><option value="all">Zdroj</option><option value="admin">Admin účty</option><option value="submission">Systémové zápisy</option></select> : null}{data.view === "emails" ? <select name="emailType" aria-label="Typ e-mailu" defaultValue={data.filters.emailType} className="min-h-11 rounded-lg border border-white/10 bg-black/20 px-3 text-sm text-white"><option value="all">Typ e-mailu</option><option value="BOOKING_CREATED">Přijetí rezervace</option><option value="BOOKING_CONFIRMED">Potvrzení rezervace</option><option value="BOOKING_CANCELLED">Zrušení rezervace</option><option value="BOOKING_RESCHEDULED">Přesun rezervace</option><option value="BOOKING_REMINDER">Připomínka termínu</option><option value="VOUCHER_SENT">Voucher</option><option value="GENERIC">Ostatní</option></select> : null}<input type="date" name="dateFrom" defaultValue={data.filters.dateFrom} aria-label="Od" className="min-h-11 rounded-lg border border-white/10 bg-black/20 px-3 text-sm text-white [color-scheme:dark]"/><input type="date" name="dateTo" defaultValue={data.filters.dateTo} aria-label="Do" className="min-h-11 rounded-lg border border-white/10 bg-black/20 px-3 text-sm text-white [color-scheme:dark]"/></>; }
