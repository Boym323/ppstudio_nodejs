...
export async function AdminMediaPage({ area, searchParams }: { area: AdminArea; searchParams?: Record<string, string | string[] | undefined> }) {
  ...
  const [mediaPage, referenceRows] = await Promise.all([listMediaPage({ page, where }), prisma.mediaCollectionItem.findMany({ where: { collection: { type: MediaCollectionType.REFERENCES } }, orderBy: [{ sortOrder: 'asc' }, { id: 'asc' }], select: { id: true, mediaAssetId: true, sortOrder: true, isVisible: true, altText: true, caption: true, mediaAsset: { select: { id: true, title: true, fileName: true, altText: true, isPublished: true, optimizedUrl: true, thumbnailUrl: true, url: true } } } })]);
  const { assets, total, pageCount } = mediaPage;
  ...
  const rows = await prisma.mediaCollectionItem.findMany({ where: { mediaAssetId: { in: assets.map((asset) => asset.id) } }, select: { mediaAssetId: true, sortOrder: true, isVisible: true, collection: { select: { type: true } } } });
  ...
  const usages = await getMediaAssetUsageBatch(assets.map((asset) => asset.id));
  ...
  return <AdminPageShell eyebrow="Obsah webu" title="Média" description="Centrální knihovna souborů a jejich skutečných použití." stats={[{ label: 'Assetů', value: String(total), tone: 'default' }, { label: 'Na stránce', value: String(assets.length), tone: 'accent' }, { label: 'Strana', value: `${displayPage}/${pageCount}`, tone: 'muted' }]} compactStats compact={area === 'salon'}>
    ...
    <AdminPanel title="Knihovna médií" description="Vyhledávejte soubory, spravujte kolekce a bezpečně nahrazujte assety." compact={area === 'salon'} denseHeader>
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center"><form action={base} className="flex min-w-0 flex-1 gap-2"><label className="sr-only" htmlFor="media-search">Hledat média</label><input id="media-search" name="q" defaultValue={search} placeholder="Hledat název, soubor nebo alt text" className="min-h-11 min-w-0 flex-1 rounded-full border border-white/10 bg-black/20 px-4 py-2 text-sm text-white outline-none placeholder:text-white/35 focus:border-[var(--color-accent)]/60"/><input type="hidden" name="usage" value={usageFilter === 'ALL' ? '' : usageFilter}/><input type="hidden" name="collection" value={category ?? ''}/><button className="min-h-11 shrink-0 rounded-full border border-white/12 px-4 py-2 text-sm text-white transition-colors hover:bg-white/5 focus-visible:outline focus-visible:outline-2 focus-visible:outline-[var(--color-accent)]">Hledat</button></form><a href="#media-upload-form" className="min-h-11 rounded-full bg-[var(--color-accent)] px-5 py-2.5 text-center text-sm font-semibold text-[var(--color-accent-contrast)] transition-opacity hover:opacity-90 focus-visible:outline focus-visible:outline-2 focus-visible:outline-[var(--color-accent)]">Nahrát</a></div>
