...
import { MediaCollectionSection, type MediaCollectionSectionItem } from '@/features/admin/components/media-collection-section';
import { ReferenceMediaSection, type ReferenceMediaItem } from '@/features/admin/components/reference-media-section';
import { getAdminMediaPreviewUrl, getManagedCollectionCanonicalUrl, getMediaAdminPath } from '@/features/admin/lib/admin-media-validation';
...
export async function AdminMediaPage({ area, searchParams }: { area: AdminArea; searchParams?: Record<string, string | string[] | undefined> }) {
  ...
  const canonicalUrl = getManagedCollectionCanonicalUrl(area, searchParams ?? {});
  ...
  const managedCollection = category === MediaCollectionType.STUDIO_GALLERY || category === MediaCollectionType.CERTIFICATES ? category : undefined;
  // Kolekce má vlastní, úplný pohled; parametry knihovny proto nemění její obsah ani stav filtrů.
  const search = managedCollection ? '' : raw('q')?.trim().toLocaleLowerCase('cs-CZ') ?? '';
  const usageFilter = managedCollection ? 'ALL' : raw('usage') === 'USED' || raw('usage') === 'UNUSED' ? raw('usage') : 'ALL';
  const publicationFilter = managedCollection ? 'ALL' : raw('publication') === 'PUBLISHED' || raw('publication') === 'HIDDEN' ? raw('publication') : 'ALL';
  ...
  const [mediaPage, publicationGroups, referenceRows, publishedAssets, managedCollectionRows, libraryAssets] = await Promise.all([managedCollection ? Promise.resolve({ assets: [], total: 0, page: 1, pageSize: 48, pageCount: 1 }) : listMediaPage({ page, where }), prisma.mediaAsset.groupBy({ by: ['isPublished'], where: { deletionRequestedAt: null }, _count: { _all: true } }), prisma.mediaCollectionItem.findMany({ where: { collection: { type: MediaCollectionType.REFERENCES } }, orderBy: [{ sortOrder: 'asc' }, { id: 'asc' }], select: { id: true, mediaAssetId: true, sortOrder: true, isVisible: true, altText: true, caption: true, mediaAsset: { select: { id: true, title: true, fileName: true, altText: true, isPublished: true, optimizedUrl: true, thumbnailUrl: true, url: true } } } }), category === MediaCollectionType.REFERENCES ? listPublishedMedia() : Promise.resolve([]), managedCollection ? prisma.mediaCollectionItem.findMany({ where: { collection: { type: managedCollection } }, orderBy: [{ sortOrder: 'asc' }, { id: 'asc' }], select: { id: true, mediaAssetId: true, sortOrder: true, isVisible: true, mediaAsset: { select: { id: true, title: true, fileName: true, altText: true, isPublished: true, optimizedUrl: true, thumbnailUrl: true, url: true } } } }) : Promise.resolve([]), managedCollection ? listMedia() : Promise.resolve([])]);
  ...
  const managedCollectionItems = managedCollectionRows.map((item): MediaCollectionSectionItem | null => {
    const asset = item.mediaAsset;
    return asset ? { ...item, mediaAsset: { id: asset.id, title: asset.title, fileName: asset.fileName, altText: asset.altText, thumbnailPublicUrl: asset.isPublished ? asset.thumbnailUrl ?? asset.optimizedUrl ?? asset.url : getAdminMediaPreviewUrl(area, asset.id), publicUrl: asset.isPublished ? asset.optimizedUrl ?? asset.url : null } } : null;
  }).filter((item): item is MediaCollectionSectionItem => item !== null);
  const managedPickerAssets = libraryAssets.filter((asset) => !managedCollectionItems.some((item) => item.mediaAssetId === asset.id)).map((asset) => ({ id: asset.id, title: asset.title, fileName: asset.fileName, altText: asset.altText, thumbnailPublicUrl: asset.thumbnailPublicUrl, publicUrl: asset.publicUrl }));
  const rows = displayAssets.length ? await prisma.mediaCollectionItem.findMany({ where: { mediaAssetId: { in: displayAssets.map((asset) => asset.id) } }, select: { mediaAssetId: true, collectionId: true, sortOrder: true, isVisible: true, collection: { select: { type: true } } } }) : [];
  ...
  const collectionHref = (collection: 'STUDIO_GALLERY' | 'CERTIFICATES') => `${base}?${new URLSearchParams({ collection })}`;
  const displayPage = mediaPage.page;
  const returnTo = managedCollection ? collectionHref(managedCollection) : href({ page: displayPage > 1 ? String(displayPage) : undefined });
  return <AdminPageShell eyebrow="Obsah webu" title="Média" description="Centrální knihovna souborů a jejich skutečných použití." stats={[{ label: 'Celkem', value: String((publicationStats.get(true) ?? 0) + (publicationStats.get(false) ?? 0)), tone: 'default' }, { label: 'Publikováno', value: String(publicationStats.get(true) ?? 0), tone: 'accent' }, { label: 'Skryto', value: String(publicationStats.get(false) ?? 0), tone: 'muted' }]} compactStats compact={area === 'salon'}>
    ...
    <AdminPanel title="Knihovna médií" description="Vyhledávejte soubory, spravujte kolekce a bezpečně nahrazujte média." compact={area === 'salon'} denseHeader>
      ...
      {managedCollection ? <MediaCollectionSection area={area} collectionType={managedCollection} items={managedCollectionItems} assets={managedPickerAssets} returnTo={returnTo}/> : null}
      {!managedCollection && category !== MediaCollectionType.REFERENCES && (displayAssets.length ? <><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4">{displayAssets.map((asset) => <MediaCard key={asset.id} area={area} asset={asset} usage={usages.get(asset.id)!} memberships={memberships.get(asset.id) ?? []} returnTo={returnTo}/>)}</div>{pageCount > 1 ? <nav aria-label="Stránkování médií" className="mt-5 flex items-center justify-between gap-3 text-sm"><PaginationControl href={href({ page: displayPage > 1 ? String(displayPage - 1) : undefined })} disabled={displayPage <= 1}>Předchozí</PaginationControl><span className="text-white/60">Strana {displayPage} z {pageCount}</span><PaginationControl href={href({ page: displayPage < pageCount ? String(displayPage + 1) : undefined })} disabled={displayPage >= pageCount}>Další</PaginationControl></nav> : null}</> : <p className="rounded-[1.35rem] border border-dashed border-white/14 p-6 text-sm text-white/62">Pro tento filtr zatím nejsou žádná média.</p>)}
    </AdminPanel>
  </AdminPageShell>;
}
...
function PaginationControl({ href, disabled, children }: { href: string; disabled: boolean; children: string }) {
  const className = `rounded-full border px-4 py-2 ${disabled ? 'opacity-40' : 'border-white/15 text-white'}`;
  return disabled ? <span className={className}>{children}</span> : <Link href={href} className={className}>{children}</Link>;
}
