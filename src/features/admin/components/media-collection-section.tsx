...
export function MediaCollectionSection({ area, collectionType, items, returnTo }: { area: AdminArea; collectionType: ManagedCollectionType; items: MediaCollectionSectionItem[]; returnTo: string }) {
  ...
  return <section className="mb-5 space-y-4 rounded-[1.25rem] border border-white/10 bg-white/[0.035] p-4 sm:p-5">
    ...
    <div className="border-t border-white/10 pt-4"><Dialog.Root open={pickerOpen} onOpenChange={setPickerOpen}><Dialog.Trigger asChild><button type="button" className="min-h-11 rounded-full border border-white/15 px-4 py-2 text-sm text-white">Přidat z knihovny</button></Dialog.Trigger><Dialog.Portal><Dialog.Overlay/><Dialog.Content className="max-w-5xl rounded-[1.7rem] border border-white/10 bg-[#131116] p-5 shadow-[0_20px_70px_rgba(0,0,0,0.45)] sm:p-6"><div className="flex items-start justify-between gap-4 border-b border-white/10 pb-4"><div><Dialog.Title>Přidat do kolekce {label}</Dialog.Title><Dialog.Description>Vyberte médium z knihovny. Přidá se na konec kolekce.</Dialog.Description></div><Dialog.Close asChild><button type="button" className="min-h-11 min-w-11 rounded-full border border-white/10 px-3 py-2 text-sm text-white/72">Zavřít</button></Dialog.Close></div><div className="mt-5"><MediaPicker area={area} enabled={pickerOpen} scope={{ type: 'COLLECTION', collectionType }} value={selectedId} selectedAsset={selectedAsset} onSelect={(asset) => { setSelectedId(asset.id); setSelectedAsset(asset); }}/></div><form action={updateMediaCollectionMembershipAction} className="mt-5 flex justify-end">{membershipFields(selectedId)}<input type="hidden" name="action" value="add"/><input type="hidden" name="isVisible" value="true"/><PendingSubmitButton disabled={!selectedId} pendingLabel="Přidávám…" className="min-h-11 rounded-full border border-white/15 px-4 py-2 text-sm text-white disabled:cursor-not-allowed disabled:opacity-40">Přidat do kolekce</PendingSubmitButton></form></Dialog.Content></Dialog.Portal></Dialog.Root></div>
  </section>;
}
