    ...
    >
      <div className="space-y-5">
        ...
        <section className="rounded-[var(--radius-panel)] border border-white/10 bg-white/[0.035] p-4 sm:p-5">
          ...
          {template.status !== "DRAFT" ? <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-white/10 pt-4"><p className="text-sm text-white/60">{template.status === "PUBLISHED" ? "Publikovaná verze zůstává neměnná. Pro další úpravy vytvořte novou verzi." : "Neaktivní verze zůstává zachovaná pro již vystavené a vytištěné vouchery. Pro nové použití vytvořte novou verzi."}</p>{template.status === "PUBLISHED" ? <DeactivateVoucherTemplateForm templateId={templateId} /> : null}</div> : null}
        ...
        {template.status === "DRAFT" ? <VoucherTemplateLayoutEditor templateId={template.id} initialLayout={voucherTemplateStoredLayoutSchema.parse(template.layout)} initialUpdatedAt={template.updatedAt.toISOString()} previewSrc={previewUrl} /> : <section className="grid gap-5 rounded-[var(--radius-panel)] border border-white/10 bg-white/[0.035] p-4 sm:p-6 md:grid-cols-[minmax(0,1fr)_minmax(16rem,0.8fr)] md:items-center"><AdminVoucherTemplatePreview src={previewUrl} alt={`Náhled šablony ${template.label}`} /><div><p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-[var(--color-accent-soft)]">{statusLabel} verze</p><h3 className="mt-2 font-display text-2xl text-white">Layout je uzamčený</h3><p className="mt-3 text-sm leading-6 text-white/65">Přesné souřadnice a typografie této verze zůstávají zachované pro vystavené vouchery. Nová verze začne jako kopie tohoto layoutu.</p><details className="mt-5"><summary className="cursor-pointer text-xs font-semibold uppercase tracking-[0.16em] text-white/55">Technické údaje layoutu</summary><pre className="mt-3 max-h-64 overflow-auto rounded-xl border border-white/8 bg-black/20 p-3 text-xs text-white/55">{JSON.stringify(template.layout, null, 2)}</pre></details></div></section>}
      </div>
    </AdminPageShell>
  );
}
