...
- Slot navázaný na jakýkoli `Booking`, včetně `CANCELLED`, se nesmí v planneru tvářit jako plain editovatelný blok. Pokud publish draft padá na hlášce `Koncept týdne se teď nepodařilo publikovat.`, zkontroluj nejdřív právě historickou vazbu `slot.bookings` a případný pokus o `deleteMany()` nad slotem chráněným FK `Booking.slotId`.
