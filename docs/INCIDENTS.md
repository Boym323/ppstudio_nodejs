...
- Release opakovaně selhává na `/api/health` s HTTP `500`, přestože `SELECT 1` funguje: příčina je v detailním read modelu e-mailové fronty nebo schema driftu. Endpoint musí degradovat na HTTP `200`, `status=warning` a `EMAIL_HEALTH_UNAVAILABLE`; skutečnou Prisma chybu načti z `journalctl -u ppstudio-web.service -n 200 --no-pager` a oprav ji dopřednou migrací či kompatibilním releasem.
...
- `GET /api/health` vrací `warning` kvůli backlogu email fronty (`pending/retrying`) bez aktivního claimu (`processing = 0`) nebo nevyřešenému recipient delivery incidentu. Pouze stale claim (`staleProcessing > 0`) je worker failure a vrací `error`/`503`; bounce či suppression konkrétní klientky zůstává `warning`/`200` a neblokuje release.
...
- Totéž rychle ověříš i přes veřejný `GET /api/health`: pokud `release.deploymentId` chybí nebo neodpovídá očekávanému commitu, runtime release env se pravděpodobně nepropsal správně.
