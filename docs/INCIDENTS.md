...
- Pokud voucherové PDF selže, ověř uložený `Voucher.templateKey`, registraci šablony a existenci masteru v `public/brand/vouchers/`; neznámý key se záměrně odmítá bez fallbacku. `SiteSettings.voucherPdfLogoMediaId` je legacy reference a aktuální renderer ji nečte.
