...
- Chybný `CRM souhrn` v detailu klientky: ověř, že read model načítá všechny rezervace klientky včetně `servicePriceFromCzk`, `scheduledStartsAt`, `scheduledEndsAt`, `VoucherRedemption.amountCzk` a `BookingPayment.amountCzk`; platební část musí dál odpovídat helperu `getBookingPaymentSummary(...)` a nesmí započítat `CANCELLED` / `NO_SHOW` do neuhrazeno.
