...
- Chybný stav v panelu `Úhrada` u rezervace: porovnej `Booking.servicePriceFromCzk`, aktuální `Service.priceFromCzk` a součet `VoucherRedemption.amountCzk` pro daný `bookingId`; panel nesmí číst hotovost/kartu ani neexistující `BookingPayment` a při neznámé ceně nesmí zobrazit `Uhrazeno`.
