...
- `MatomoTracker` drží externí `matomo.js` na `next/script` strategii `lazyOnload`, protože veřejný web nepotřebuje třetí stranu v kritickém renderu; samotný `_paq` bootstrap ale probíhá hned po hydraci v client efektu, aby tokenové self-service route měly bezpečnou queue k dispozici ještě před první akcí.
- Pageview tracking v App Routeru poslouchá `usePathname()` a `useSearchParams()`. Po první hydraci nejdřív zbootstrapuje `_paq` (`setTrackerUrl`, `setSiteId`, `setCustomUrl`) a pageview pošle jen na routách, kde je povolený; další klientské navigace posílá se sanitizovanou URL.
