...
- Server-side dashboard reporting je v `src/lib/analytics/matomo.ts` a je záměrně oddělený od klientského helperu. Používá pouze `MATOMO_*` env bez `NEXT_PUBLIC_`, `import "server-only"`, Matomo metody `VisitsSummary.get`, `Goals.get`, `Events.getAction` a `Referrers.getReferrerType`, a každý request cachuje přes `fetch(url, { next: { revalidate: 300 } })`.
- `getDashboardAnalytics()` mapuje Matomo event labels `Booking / Service selected`, `Booking / Date selected`, `Booking / Time selected` a `Booking / Created` do funnel polí `service`, `date`, `time`, `created`; protože `Events.getAction` může vracet jen action label, agregace akceptuje i zkrácené labely `Service selected`, `Date selected`, `Time selected` a `Created`.
...
- `src/components/admin/AnalyticsWidget.tsx` je klientská komponenta pro admin dashboard. Na mountu volá `fetch('/api/admin/analytics')`, validuje shape payloadu v runtime, zobrazuje `Načítání…` / `Data nejsou dostupná` a po úspěchu vykreslí KPI grid, top source a vertikální funnel.
...
- Dashboard analytics API nově vrací také `sources`. Backend čte `Referrers.getCampaigns` pro kampaně a při absenci kampaní používá `Referrers.getReferrerType`; labely mapuje na business názvy `Instagram`, `Firmy`, `Google`, `Přímý vstup`, případně `Offline`/`Ostatní`. Konverze u zdrojů jsou odhad: počet `Booking / Created` se rozdělí podle podílu návštěv jednotlivých zdrojů.
