...
- Read model seznamu rezervací už nepoužívá `Dnes / Zítra / Později / Dříve`; seskupení je `pending / upcoming / past`, kde `pending` vítězí vždy bez ohledu na datum. `Past` obsahuje historické i uzavřené stavy (`COMPLETED`, `CANCELLED`, `NO_SHOW`) a `upcoming` drží ostatní aktivní rezervace od dneška dál.
...
- Běžné platby k rezervaci jsou samostatný ledger `BookingPayment`; nepřepisuj jím voucher redemption model. Stav úhrady se neukládá do DB, počítá ho `getBookingPaymentSummary(...)` z `Booking.servicePriceFromCzk`, `VoucherRedemption.amountCzk` a `BookingPayment.amountCzk`. Server actions pro zápis/mazání plateb jsou v `src/features/bookings/actions/booking-payment-actions.ts` a UI patří přímo do existující sekce `Úhrada` v detailu rezervace.
