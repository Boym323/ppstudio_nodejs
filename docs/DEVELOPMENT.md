...
- `package.json` používá SemVer `MAJOR.MINOR.PATCH`; aktuální release je `0.3.11` v pre-stable řadě.
...
- Sekce dashboardu `Vyžaduje pozornost` se renderuje pouze při existenci actionable alertů (`alert.emphasis !== "ok"`); pokud jsou jen `ok` stavy, komponenta vrací `null`.
...
- Actionable alert pravidla v `getAdminDashboardData(...)` jsou záměrně provozní a konzervativní: `pending-bookings`, `email-failures`, `nearby-slots`, `today-no-free-window` (dnes jsou rezervace, ale bez volného okna), `current-overdue` (aktivní rezervace po konci termínu) a `week-capacity-low` (obsazenost týdne alespoň 80 % a zároveň nejvýše 3 volné sloty).
...
- Vizuální priorita panelu `Úhrada` je záměrně `Stav úhrady -> Cena k úhradě -> + Zapsat platbu -> + Uplatnit voucher -> Přehled úhrad`. Horní `PaymentSummaryBlock` je dominantní a obsahuje i kompaktní vstup do individuální úpravy ceny přes `Upravit` u položky `Cena k úhradě`; samostatný viditelný `BookingPriceBlock` se ve výchozím zobrazení nepoužívá. `+ Zapsat platbu` drž jako primární CTA, ale existující běžné platby nevypisuj ve zvláštním bloku mimo ledger; patří jednou do `Přehled úhrad` spolu s voucher redemptions a případným smazáním platby. Voucher akci drž jako sekundární/outline přímo v kompaktním voucher bloku a prázdný `Přehled úhrad` vizuálně tlumený.
...
- `BookingManagementPanel` pro veřejný self-service přesun používá u cílových sekcí `scroll-mt-*` a v `selectDate` / `selectSlot` okamžitý `scrollIntoView`. Nepřepínej tyto programové skoky zpět na smooth scroll bez ověření Playwrightem, protože rychlé klikání na vzdálenější sloty může proběhnout během animace a prvek pak není stabilní ve viewportu.
...
- Hlavní seznam emailů záměrně nezobrazuje placeholder tracking pole `Otevřeno` a `Kliknuto` jako plné sloupce; dokud tracking nemá reálná data, používá se jen nenápadný badge `Tracking připraven`.
