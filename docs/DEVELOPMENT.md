...
- Vizuální priorita panelu `Úhrada` je záměrně `Stav úhrady -> Cena k úhradě -> + Zapsat platbu -> + Uplatnit voucher -> Přehled úhrad`. Horní `PaymentSummaryBlock` je dominantní a obsahuje i kompaktní vstup do individuální úpravy ceny přes `Upravit` u položky `Cena k úhradě`; samostatný viditelný `BookingPriceBlock` se ve výchozím zobrazení nepoužívá. `+ Zapsat platbu` drž jako primární CTA, voucher akci jako sekundární/outline a prázdný `Přehled úhrad` vizuálně tlumený.
