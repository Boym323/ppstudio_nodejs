...
- Vizuální priorita panelu `Úhrada` je záměrně `Stav úhrady -> + Zapsat platbu -> + Uplatnit voucher -> Cena rezervace -> Přehled úhrad`. Horní `PaymentSummaryBlock` je dominantní, `+ Zapsat platbu` drž jako primární CTA a voucher akci jako sekundární/outline. `BookingPriceBlock` je jen podpůrná editační zóna (ne hlavní provozní akce) a prázdný `Přehled úhrad` má být vizuálně tlumený.
