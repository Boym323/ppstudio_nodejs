...
- Pageview tracking v App Routeru poslouchá `usePathname()` a `useSearchParams()`. Layout efekt posílá první i další povolené pageview se sanitizovanou URL a deduplikuje je podle poslední skutečně odeslané cesty; inline bootstrap pouze nastaví tracker a bezpečnou URL.
