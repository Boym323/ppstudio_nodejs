...
- PDF voucheru běží přes route handlery `/admin/vouchery/[voucherId]/pdf` a `/admin/provoz/vouchery/[voucherId]/pdf`, sdílený handler `createAdminVoucherPdfRoute(...)` a server-only generátor `src/features/vouchers/lib/voucher-pdf.ts`. Handler vždy ověřuje session a roli `OWNER` nebo `SALON`, PDF neukládá do DB ani na disk a generuje ho z aktuálního read modelu voucheru.
...
- Server action `sendVoucherEmailAction(...)` vždy ověřuje session i roli `OWNER` nebo `SALON`; samotný enqueue core je v `queueVoucherEmailLog(...)` a validuje `voucherId`, `recipientEmail`, `subject`, `message` přes Zod.
...
- Šablona `voucher-sent-v1` je napojená v `src/lib/email/templates.ts` a používá server-side helper `buildVoucherEmailTemplate(...)` z `src/features/vouchers/lib/voucher-email-template.ts`. Příloha je PDF z `generateVoucherPdf(...)` (bez HTTP callu na admin route), filename `voucher-KOD.pdf`, content type `application/pdf`.
