...
- PDF generátor používá `pdf-lib`, `qrcode`, `@pdf-lib/fontkit` a Noto Sans z `@fontsource/noto-sans`, aby bezpečně fungovala česká diakritika bez commitování fontů do repozitáře. QR kód míří na budoucí veřejnou URL `/vouchery/overeni?code=...` nad `siteConfig.url`.
