...
- Ve veřejném booking flow drž dva typy Matomo měření odděleně: hlavní funnel eventy (`Zobrazena`, `Služba vybrána`, `Čas vybrán`, `Kontakt zahájen`, `Odeslána rezervace`, `Vytvořena`) a diagnostické mikro-eventy (`Datum vybráno`, `Formulář chyba`, `Kontakt pole fokus`, `Kontakt pole vyplnění začátek`, `Kontakt pole chyba`). Do event `name` neposílej obsah formuláře ani raw chybové texty.
