...
- Doporučené termíny používají dva privacy-safe Matomo eventy: `Rezervace / Doporučené termíny zobrazeny / service-slug` se pošle jednou pro skutečně renderovanou konkrétní sadu a `Rezervace / Doporučený termín vybrán / YYYY-MM-DD | HH:mm–HH:mm | service-slug | pozice N` pouze při kliknutí na slot se stejným canonical `key`. Podíl eventových výběrů doporučení je `Doporučený termín vybrán / Čas vybrán`; eventové CTR doporučení je `Doporučený termín vybrán / Doporučené termíny zobrazeny`. Nejde automaticky o podíl unikátních klientek ani sessions.
