...
- `package.json` používá SemVer `MAJOR.MINOR.PATCH`; aktuální release je `0.3.2` v pre-stable řadě.
...
- Veřejné JSON-LD upravuj přes `src/features/public/components/seo-json-ld.tsx`; data ber z public read modelů a `getPublicSalonProfile()`, ne z duplicitních hardcoded kontaktů. Testuj serializer i helpery v `seo-json-ld.test.ts`, zejména CZK cenu, ISO duration a absenci prázdných hodnot.
