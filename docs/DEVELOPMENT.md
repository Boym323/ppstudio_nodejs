...
- Resend delivery taxonomie: `email.bounced`, `email.failed` a `email.suppressed` jsou delivery failures (`Nedoručeno`) a vstupují do aktivního počtu delivery incidentů; `email.complained` znamená, že příjemce označil e-mail jako spam. Complaint je samostatný reputační warning (`Nahlášeno jako spam`) v historii a `Pozornosti`, ale není delivery failure, nemění `/api/health` na `503` a resend ji automaticky neuzavírá. Všechny tyto provider eventy posílají owner Pushover jen při prvním zapsání konkrétního timestampu na `EmailLog`, aby se předešlo spamování při opakovaných webhook retries.
