...
- Voucherová PDF používají registry šablon v `src/features/vouchers/lib/voucher-template-registry.ts`. Každý voucher ukládá přesný `templateKey`; generátor historického voucheru nefallbackuje na aktuální default a aktivní šablony pro nové vouchery se odvozují z registry. Master PDF, preview PNG a layout každé šablony tvoří neměnný verzovaný celek: redesign znamená nový key (`classic-v2`), nikoli přepsání geometrie `classic-v1`. Grafický základ voucheru je verzovaný PDF master; logo a pevné kontaktní/grafické údaje jsou jeho součástí a renderer doplňuje pouze dynamická voucherová data.
...
- Voucherový PDF renderer nečte kontaktní údaje ze `SiteSettings`: při načtení masteru ověří jeho rozměr podle registry a poté doplní pouze dynamická data voucheru. Kontakty pro web a e-maily nadále používají `SiteSettings`; bezpečná textová doména v těchto výstupech má prioritu `VOUCHER_PUBLIC_DOMAIN` -> `NEXT_PUBLIC_SITE_DOMAIN` -> hostname z `NEXT_PUBLIC_APP_URL`.
