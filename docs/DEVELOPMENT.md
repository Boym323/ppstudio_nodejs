...
- Panel `Úhrada` v admin detailu rezervace je read-only settlement summary nad existujícím voucher čerpáním. `getAdminBookingDetailData(...)` vrací `voucher.paymentSummary`: `totalPriceCzk` bere ze snapshotu `Booking.servicePriceFromCzk`, fallbackově z aktuální `Service.priceFromCzk`; `voucherPaidCzk` je součet `VoucherRedemption.amountCzk`; `paidAmountCzk` je zatím stejné jako voucherová úhrada; `remainingAmountCzk` je `max(totalPriceCzk - paidAmountCzk, 0)`. Při neznámé ceně UI ukazuje `Cena není nastavena`, `remainingAmountCzk` je `null` a stav nesmí být `PAID`. Prezentačně má panel zůstat kompaktní: tři metriky `Cena služby / Uhrazeno voucherem / Zbývá doplatit`, badge stavu úhrady, nejvýraznější doplatek, jemná poznámka `V této verzi evidujeme pouze úhrady voucherem.` a ruční formulář schovaný za `+ Uplatnit voucher`, pokud rezervace nemá intended voucher ani redemption.
