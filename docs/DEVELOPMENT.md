...
- Voucherová PDF používají registry šablon v `src/features/vouchers/lib/voucher-template-registry.ts`. Každý voucher ukládá přesný `templateKey`; generátor historického voucheru nefallbackuje na aktuální default a aktivní šablony pro nové vouchery se odvozují z registry. Přidání nové šablony probíhá v tomto pořadí: (1) přidej neměnný master PDF asset do `public/brand/vouchers/`, (2) přidej odpovídající preview PNG, (3) zaregistruj `masterAssetKey` a reader v `src/features/vouchers/lib/voucher-master-assets.ts`, (4) přidej úplnou template definition do `voucher-template-registry.ts`, včetně `allowedTypes` a layoutu, (5) zaměř a ověř všechny souřadnice, page boxes a typografii v layoutu a (6) doplň registry, PDF renderer a preflight/regresní testy. Master PDF, preview PNG a layout každé šablony tvoří neměnný verzovaný celek: existující template version se nepřepisuje; nový design znamená nový key (`classic-v2`, `christmas-v1`), nikoli změnu geometrie `classic-v1`. Grafický základ voucheru je verzovaný PDF master; logo a pevné kontaktní/grafické údaje jsou jeho součástí a renderer doplňuje pouze dynamická voucherová data.
- Každá template definition nese vlastní `layout`, včetně page size, TrimBoxu, dynamických oblastí, souřadnic QR a typografických parametrů. Renderer je obecný a pouze řeší `templateKey -> registry -> master + layout`; neobsahuje větvení pro konkrétní designy. DIGITAL a PRINT používají stejný layout, DIGITAL je pouze vektorový crop PRINT stránky. Přidání nové šablony proto znamená přidat master, preview a jednu registry definition; formulář vytvoření i Nastavení → Vouchery ji načtou automaticky z aktivních definition. `allowedTypes` filtruje UI a je znovu vynuceno serverem.
