...
- Strukturovaná data jsou v `src/features/public/components/seo-json-ld.tsx`: veřejný layout vkládá `BeautySalon`/`WebSite`, detail služby vkládá `Service` a `BreadcrumbList`. JSON-LD se serializuje přes `JSON.stringify(...).replace(/</g, "\\u003c")`; adresa salonu v `BeautySalon` se bere z `getPublicSalonProfile()` / `SiteSettings`, aby držela stejnou hodnotu jako veřejný kontakt.
