...
- `package.json` používá SemVer `MAJOR.MINOR.PATCH`; aktuální release je `0.7.0` v pre-stable řadě.
...
- Pro standardní produkční rollout používej `./deploy/release.sh`; skript sjednocuje kroky `pull -> staging npm ci --include=dev -> db:generate -> db:check-migrations -> prisma migrate deploy -> lint -> build -> krátký stop/swap/start systemd služeb`, předem ověří nainstalované units `ppstudio-web.service` a `ppstudio-email-worker.service` a zastaví se i při nalezených legacy PM2 procesech `ppstudio-web` / `ppstudio-email-worker`.
...
- Když upravuješ `GET /api/health`, zachovej ho jako čistě veřejný provozní snapshot bez citlivých detailů: release metadata (`deploymentId`, `deploymentVersion`, `gitHash`) a `durationMs` jsou žádoucí, ale raw e-mailové chyby nebo tokeny do payloadu nesmí.
...
- Playwright smoke test pro veřejné stránky ověřuje, že canonical a `og:url` míří na stejnou stránkovou URL. Discovery test zároveň hlídá `Host`/`Sitemap` v `robots.txt`, sitemap `<loc>` na stejném originu a absenci historické `http://ppstudio.cz` URL.
...
- `getDashboardAnalytics()` mapuje Matomo event labels `Rezervace / Zobrazena`, `Rezervace / Služba vybrána`, `Rezervace / Čas vybrán`, `Rezervace / Kontakt zahájen`, `Rezervace / Odeslána rezervace` a `Rezervace / Vytvořena` do funnel polí `viewed`, `service`, `term`, `contact`, `submitted`, `created`; protože `Events.getAction` může vracet jen action label, agregace akceptuje i zkrácené labely bez prefixu `Rezervace / ...`. Hlavní KPI `conversions` je stejné číslo jako `funnel.created`, aby se metrika rezervací nerozcházela s funnel krokem.
...
- Ve veřejném booking flow drž dva typy Matomo měření odděleně: hlavní funnel eventy (`Zobrazena`, `Služba vybrána`, `Čas vybrán`, `Kontakt zahájen`, `Odeslána rezervace`, `Vytvořena`) a diagnostické/provozní mikro-eventy (`Datum vybráno`, `Formulář chyba`, `Termín konflikt při odeslání`, `Bez služeb`, `Bez termínů`, `Kontakt pole fokus`, `Kontakt pole vyplnění začátek`, `Kontakt pole chyba`). Do event `name` neposílej obsah formuláře ani raw chybové texty.
- Pro tokenové self-service route drž stejné bezpečnostní pravidlo: lze měřit jen neosobní lifecycle kroky jako `Změna termínu otevřena`, `Změna termínu odeslána`, `Storno odesláno`, `Storno dokončeno`, ale nikdy ne token, plnou URL, jméno klientky ani kontakt.
...
- Search input v rezervacích používá malý klientský combobox s debounce fetch do `/api/admin/bookings/search`. Endpoint musí zůstat admin-only a vracet bezpečné textové návrhy bez přímé mutace URL nebo DB. Pro běžný text nabídne hlavně `fullName` a název služby; kontaktní návrhy (e-mail/telefon) se mají ukázat jen když dotaz vypadá jako kontakt.
...
- Lookup endpoint podporuje i read-only `GET /api/admin/vouchers/lookup?voucherCode=...` (kromě `POST`) a completion panel používá primárně tuto variantu s `cache: no-store`.
...
- Veřejné ověření voucheru běží na `/vouchery/overeni?code=...`, má `noindex` metadata a záměrně není v `sitemap.ts`. Používá `verifyVoucherPublic(...)`, smí zobrazit jen kód, typ, zbývající hodnotu u `VALUE`, název služby ze snapshotu u `SERVICE` a platnost do; neplatným kódům vrací pouze bezpečné důvody bez citlivých polí. Následné CTA po platném výsledku může vést na rezervaci nebo veřejný kontakt studia, ale nesmí spouštět uplatnění ani zapisovat voucher data.
...
- `pretest:e2e` pro Playwright build musí používat stejný build-time origin jako následný `next start`: nastavuje `NEXT_PUBLIC_APP_URL=${PLAYWRIGHT_BASE_URL:-http://127.0.0.1:3100}` a zároveň dummy Meta Pixel env (`NEXT_PUBLIC_META_PIXEL_ENABLED=true`, `NEXT_PUBLIC_META_PIXEL_ID=123456789`). Jinak admin login/logout redirecty a proxy ochrana v produkčním buildu zůstanou inlinované na jiný host a browser testy po přihlášení skončí cross-origin `ERR_CONNECTION_REFUSED`.
