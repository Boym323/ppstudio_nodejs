...
- Kontaktní údaje ve voucher PDF se berou ze `SiteSettings` přes `getSiteSettings()` a `getSalonAddressLine(...)`; web se zobrazuje z `siteConfig.url`. Prázdné kontaktní řádky se v PDF nevypisují.
