...
- Veřejná prezentační route `/vouchery` je indexovatelná landing page pro akvizici a rozhodnutí před nákupem voucheru. Patří do `sitemap.ts`, používá `buildPageMetadata(...)`, čte aktuální veřejné služby z `getPublicServices()` a má vést jen na bezpečné veřejné kroky (`kontakt`, `mailto:`, `tel:`, `/vouchery/overeni`, případně detail služby), ne na admin nebo přímé čerpání voucheru.
