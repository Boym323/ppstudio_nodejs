...
- `package.json` používá SemVer `MAJOR.MINOR.PATCH`; aktuální release je `3.6.0` ve stabilní řadě.
...
- Výchozí `npm run dev` používá Next.js 16 dev server přes Webpack, protože Turbopack HMR v developmentu může po invalidaci nebo restartu hlásit `ChunkLoadError` z interního `hmr-client` chunku. Turbopack lze stále spustit explicitně přes `npm run dev:turbopack`.
...
- Pokud dev server spadne na poškozené Turbopack cache (`Failed to restore task data`, chybějící `.sst` v `.next/dev/cache/turbopack`), použij:
  ...
  - `npm run dev:webpack` (explicitní Webpack režim; stejně jako výchozí `npm run dev`)
