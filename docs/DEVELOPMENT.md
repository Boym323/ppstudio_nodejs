...
- Pro standardní produkční rollout používej `./deploy/release.sh`; skript vytváří celý release v `releases/<commit>-<čas>` a atomicky přepíná `current`. Pořadí je `pull -> staging npm ci -> generate -> kontrola historie -> prisma validate -> lint -> typecheck -> test:release -> build bez DB čtení aplikace -> migrate deploy -> stop -> přepnutí current -> start webu i workeru -> health + smoke`. `test:release` nespouští DB integrační scénáře nad produkčním `.env`; ty zůstávají povinnou CI bránou. Migrace tedy smějí být jen expand/contract kompatibilní s předchozím releasem; rollback automaticky vrací pouze celý runtime release, nikdy DB schéma.
...
- Veřejné API obsahuje i route handler `GET /api/health` pro externí monitoring webu: endpoint dělá DB check (`SELECT 1`), čte stav email outbox fronty a aktivní delivery incidenty odděleně v `emailIncidents`; vrací agregovaný stav `ok|warning|error` + `alerts`, přičemž pouze `error` odpovídá HTTP `503`.
...
- CI workflow `.github/workflows/ci.yml` běží na push do `main`/`master` a na pull requesty. Hlavní kontroly jsou samostatné joby `lint`, `typecheck`, `test`, `build`, `e2e`, `e2e chromium shard 2`, `e2e mobile` a `e2e mobile shard 2`, takže GitHub UI i branch protection vidí každý check zvlášť. Job `test` po PostgreSQL service containeru, `prisma migrate deploy` a generování klienta spustí `npm run test:ci`: coverage unit vrstvy a následně sériové DB integrace; coverage report je jeho artefakt. Čtyři E2E joby každý připraví vlastní PostgreSQL a build a spustí jeden shard Playwrightu: Chromium 1/2 a 2/2, mobile Chrome 1/2 a 2/2. Job `lint` navíc bez PostgreSQL spouští `npm run test:release-script`.
...
- Repo-level enforcement zůstává mimo git: po přidání nebo přejmenování workflow ručně slad branch protection / rulesets a required status checks v GitHub nastavení repozitáře, jinak budou nové kontroly jen informativní. Required checks hlavního CI mají mířit na přesné GitHub job names `lint`, `typecheck`, `test`, `build`, `e2e`, `e2e chromium shard 2`, `e2e mobile` a `e2e mobile shard 2`; `coverage` sem nepatří, protože samostatný status check nevytváří.
...
- Chybové Resend webhook eventy (`email.bounced`, `email.complained`, `email.failed`, `email.suppressed`) navazují na owner Pushover alert (`EMAIL_FAILED`); jde o následný problém doručení po předání providerovi, nikoli o vyčerpání worker retry. Dispatch se spouští jen při prvním zapsání konkrétního chybového timestampu na `EmailLog`, aby se předešlo spamování při opakovaných webhook retries.
- `EmailLog` zachovává historický transportní i provider delivery stav. Ruční resend zakládá explicitní chain (`resendOfId`, stabilní `resendRootId`); teprve webhook `email.delivered` pro resend idempotentně uzavře incident kořenové zprávy. Bounce tedy zůstane v historii jako nedoručený, ale po následně doručeném resendu už nepatří do `Pozornost` ani do počtu aktivních e-mailových problémů. Samotný `SENT`, další bounce nebo jiný lifecycle e-mail incident neuzavírá.
