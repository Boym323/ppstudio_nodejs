...
- Voucherová PDF používají registry šablon v `src/features/vouchers/lib/voucher-template-registry.ts`. Každý voucher ukládá přesný `templateKey`; generátor historického voucheru nefallbackuje na aktuální default a aktivní šablony pro nové vouchery se odvozují z registry. Master PDF, preview PNG a layout každé šablony tvoří neměnný verzovaný celek: redesign znamená nový key (`classic-v2`), nikoli přepsání geometrie `classic-v1`.
...
- `SiteSettings.voucherPdfLogoMediaId` zůstává v DB kvůli zpětné kompatibilitě, ale aktuální admin UI ani voucherový PDF/email generator ho nepoužívají. Grafika voucheru včetně loga je součástí verzovaného master PDF a PDF šablony se do Media Manageru nepřidávají.
