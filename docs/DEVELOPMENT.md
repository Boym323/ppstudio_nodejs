...
- Veřejná prezentační route `/vouchery` je indexovatelná landing page pro akvizici a rozhodnutí před nákupem voucheru. Patří do `sitemap.ts`, používá `buildPageMetadata(...)`, pro blok doporučených služeb má číst jen úzký výběr přes `getVoucherSuggestedServices(3)` místo celého katalogu a má vést jen na bezpečné veřejné kroky (`kontakt`, `mailto:`, `tel:`, `/vouchery/overeni`, případně detail služby), ne na admin nebo přímé čerpání voucheru.
