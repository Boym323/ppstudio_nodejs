...
- `package.json` používá SemVer `MAJOR.MINOR.PATCH`; aktuální release je `0.5.0` v pre-stable řadě.
...
- Pokud browser v dev režimu po restartu nebo invalidaci layoutu skončí na `ChunkLoadError: Failed to load chunk /_next/static/chunks/...`, root shell teď provede jeden automatický hard reload už z inline `beforeInteractive` guardu v root layoutu, takže ochrana funguje i když spadne samotný klientský layout chunk. Když ani druhé načtení nepomůže, vyčisti `.next` přes `npm run dev:clean`; nejde typicky o business chybu `/rezervace`, ale o rozjetý HMR/client cache stav.
...
- `getDashboardAnalytics()` mapuje Matomo event labels `Rezervace / Služba vybrána`, `Rezervace / Datum vybráno`, `Rezervace / Čas vybrán` a `Rezervace / Vytvořena` do funnel polí `service`, `date`, `time`, `created`; protože `Events.getAction` může vracet jen action label, agregace akceptuje i zkrácené labely `Služba vybrána`, `Datum vybráno`, `Čas vybrán` a `Vytvořena`. Hlavní KPI `conversions` je stejné číslo jako `funnel.created`, aby se metrika rezervací nerozcházela s funnel krokem.
