...
- Aplikace pro IP rate limiting používá výhradně `X-Real-IP`. Nginx Proxy Manager proto musí aplikaci zpřístupňovat jako jediná reverse proxy a pro každou proxovanou žádost tuto hlavičku přepsat skutečnou vzdálenou adresou, například `proxy_set_header X-Real-IP $remote_addr;`. Na `X-Forwarded-For` se nelze spoléhat, protože klient může jeho hodnotu podvrhnout. Pokud je před Nginx Proxy Managerem další proxy (např. CDN), nastav v Nginxu důvěryhodné adresy této vrstvy a obnov `$remote_addr` přes `real_ip_header` / `set_real_ip_from`.
