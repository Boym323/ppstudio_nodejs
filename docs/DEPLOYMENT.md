...
- Verzionovaná web unit aktuálně binduje Next.js na `0.0.0.0:3000`. Repo neobsahuje konfiguraci NPM/Nginxu ani firewall, proto z něj nelze rozhodnout, zda proxy běží lokálně nebo vzdáleně. Přímý přístup klienta na port 3000 by dovolil podvrhnout `X-Real-IP` a obejít IP část limitů. Je-li proxy lokální, v `deploy/systemd/ppstudio-web.service` i `.example` nastav `Environment=HOSTNAME=127.0.0.1` a ověř upstream na loopbacku. Je-li vzdálená, firewall musí povolit TCP/3000 pouze z reálných IP/CIDR proxy a zakázat jej všem ostatním zdrojům, včetně ekvivalentního IPv6 pravidla. Nevymýšlej zástupné adresy.
